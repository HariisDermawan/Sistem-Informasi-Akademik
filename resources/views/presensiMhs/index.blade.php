...
<div class="bg-white rounded-xl shadow-md p-6 mb-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
            <h2 class="text-xl font-bold text-gray-800">Presensi Mahasiswa</h2>
            <p class="text-sm text-gray-500 mt-1">Kelola data presensi mahasiswa & scan QR untuk absensi</p>
        </div>

        <div class="flex gap-3 mt-4 md:mt-0">
            <!-- Button Scan QR -->
            <button id="openScanQrMahasiswaBtn"
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div class="relative w-full md:w-80">
            <input type="text" placeholder="Cari mahasiswa / NIM / mata kuliah..."
        ...
        <table class="w-full border border-gray-200">
            <thead>
                <tr class="bg-gray-50 border-b">
                    ...
                    <th class="text-left py-3 px-4 font-medium text-gray-700">Mahasiswa</th>
                    <th class="text-left py-3 px-4 font-medium text-gray-700">NIM</th>
                    <th class="text-left py-3 px-4 font-medium text-gray-700">Mata Kuliah</th>
                    <th class="text-left py-3 px-4 font-medium text-gray-700">Tanggal</th>
                    <th class="text-left py-3 px-4 font-medium text-gray-700">Jam</th>
            ...
            <tbody id="presensiMahasiswaTbody">
                <!-- Contoh row -->
                <tr class="border-b hover:bg-gray-50">
                    <td class="py-4 px-4 text-gray-700">1</td>
                    <td class="py-4 px-4 text-gray-700">Ahmad Ramadhan</td>
                    <td class="py-4 px-4 text-gray-700">2101234567</td>
                    <td class="py-4 px-4 text-gray-700">Pemrograman Web</td>
                    <td class="py-4 px-4 text-gray-700">2026-01-29</td>
                    <td class="py-4 px-4 text-gray-700">08:00</td>
                    <td class="py-2 px-4 text-green-800 font-medium ">Hadir</td>
                    <td class="py-4 px-4 text-gray-700">Tepat waktu</td>
                    <td class="py-4 px-4">
...
<!-- MODAL SCAN QR MAHASISWA FULL CAMERA -->
<div id="scanQrMahasiswaModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-2xl h-[80vh] p-6 relative flex flex-col">
        <button id="closeScanQrMahasiswaBtn" class="absolute top-3 right-3 text-gray-500 hover:text-gray-800 z-10">
            <i class="fas fa-times text-lg"></i>
        </button>

        <h3 class="text-lg font-bold text-gray-800 mb-2 z-10">Scan QR Presensi Mahasiswa</h3>
        <p class="text-sm text-gray-500 mb-4 z-10">Arahkan kamera ke QR Code untuk absensi otomatis</p>

        <!-- Video Kamera Full -->
        <div class="flex-1 w-full">
            <video id="qrMahasiswaVideo" class="w-full h-full rounded-lg border object-cover" autoplay></video>
        </div>
    </div>
</div>

<script>
const openScanQrMahasiswaBtn = document.getElementById('openScanQrMahasiswaBtn');
const scanQrMahasiswaModal = document.getElementById('scanQrMahasiswaModal');
const closeScanQrMahasiswaBtn = document.getElementById('closeScanQrMahasiswaBtn');
const qrMahasiswaVideo = document.getElementById('qrMahasiswaVideo');
let mahasiswaStream;

// Buka kamera saat klik Scan QR Mahasiswa
openScanQrMahasiswaBtn.addEventListener('click', async () => {
    scanQrMahasiswaModal.classList.remove('hidden');
    scanQrMahasiswaModal.classList.add('flex');

    try {
        mahasiswaStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } });
        qrMahasiswaVideo.srcObject = mahasiswaStream;
    } catch (err) {
        alert("Tidak bisa membuka kamera: " + err);
        scanQrMahasiswaModal.classList.add('hidden');
        scanQrMahasiswaModal.classList.remove('flex');
    }
});
...
const closeMahasiswaModal = () => {
    if(mahasiswaStream){
        mahasiswaStream.getTracks().forEach(track => track.stop());
    }
    scanQrMahasiswaModal.classList.add('hidden');
    scanQrMahasiswaModal.classList.remove('flex');
};

closeScanQrMahasiswaBtn.addEventListener('click', closeMahasiswaModal);
scanQrMahasiswaModal.addEventListener('click', e => {
    if(e.target === scanQrMahasiswaModal) closeMahasiswaModal();
});
</script>
